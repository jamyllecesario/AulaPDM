...
export default function JogoDoBichoScreen() {
  ...
  return (
    <View style={{ flex: 1, padding: 16 }}>
      <Button mode="contained" onPress={gerarAnimal} style={{ marginBottom: 16 }}>
        Gerar Animal do Jogo do Bicho
      </Button>
      {animal && (
        <Card>
          <Card.Content>
            <Text variant="titleLarge">{animal.nome}</Text>
            <Image source={{ uri: animal.imagem }} style={{ width: '100%', height: 200, marginTop: 10 }} />
          </Card.Content>
        </Card>
      )}
    </View>
